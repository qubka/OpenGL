...
GLint glxGpuTotalMemory() {
    GLint totalMemKb = 0;
    glCall(glGetIntegerv, GL_GPU_MEM_INFO_TOTAL_AVAILABLE_MEM_NVX, &totalMemKb);
...
GLint glxGpuAvailMemory() {
    ...
    glCall(glGetIntegerv, GL_GPU_MEM_INFO_CURRENT_AVAILABLE_MEM_NVX, &curAvailMemKb);

    if (curAvailMemKb == 0) {
        glCall(glGetIntegerv, GL_TEXTURE_FREE_MEMORY_ATI, &curAvailMemKb);
    }
